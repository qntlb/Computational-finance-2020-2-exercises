...
/**
 * This class represents an exchange option on two LIBOR rates: one LIBOR rate from time
 *  periodStartFirstLibor to time periodEndFirstLibor, and one from periodStartSecondLibor
 *  to periodEndSecondLibor. The product is payed at periodEndSecondLibor. Note the
...
public class MyExchangeOption extends AbstractLIBORMonteCarloProduct{
	...
	public MyExchangeOption(double periodStartFirstLibor, double periodEndFirstLibor,
			double periodStartSecondLibor,	double periodEndSecondLibor) {
		super();
	 ...
	 * LIBORModelMonteCarloSimulationModel. From this underlying, we take the two LIBOR rates involved.
